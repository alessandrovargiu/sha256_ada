...
package SHA256_TYPES is
    ...
    subtype LONG_WORD is Interfaces.Unsigned_64;


    type UNSGN_BYTE_TYPE_ARRAY is array (Natural range <>) of BYTE;

    type UNSGN_WORD_TYPE_ARRAY64 is array (0 .. 63) of WORD;
    type UNSGN_WORD_TYPE_ARRAY8 is array (0 .. 7) of WORD;
    type UNSGN_BYTE_TYPE_ARRAY32 is array (0 .. 31) of BYTE;  
    type UNSGN_BYTE_TYPE_ARRAY64 is array (0 .. 63) of BYTE;  
    
    -- Context record containing state of the computation
    type SHA256_CTX is record
        data : UNSGN_BYTE_TYPE_ARRAY64;
        datalen : WORD;
        bitlen: LONG_WORD;
        state : UNSGN_WORD_TYPE_ARRAY8;
    end record;

    -- First 32 bits of the fractional parts of the square roots
    -- of the first 8 prime numbers
    H8: constant UNSGN_WORD_TYPE_ARRAY8 := 
    (
        0 => 16#6a09e667#, 
        1 => 16#bb67ae85#, 
        2 => 16#3c6ef372#, 
    ...
    K64: constant UNSGN_WORD_TYPE_ARRAY64 :=
    (
   ...
   -- Function prototypes
    function sig0(x: in WORD) return WORD;

	function sig1(x: in WORD) return WORD;

	function ep0(x: in WORD) return WORD;

	function ep1(x: in WORD) return WORD;

	function Ch(x: in WORD;
				y: in WORD;
				z: in WORD) return WORD;

	function Maj(x: in WORD;
				y: in WORD;
				z: in WORD) return WORD;

    function To_Hex(Value : BYTE) return String;

    function To_Hex(Value : WORD) return String;
